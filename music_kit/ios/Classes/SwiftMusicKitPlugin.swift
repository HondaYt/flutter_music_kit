...
public class SwiftMusicKitPlugin: NSObject, FlutterPlugin {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    let channel = FlutterMethodChannel(
      name: "plugins.misi.app/music_kit", binaryMessenger: registrar.messenger())
    let instance = SwiftMusicKitPlugin()
    registrar.addMethodCallDelegate(instance, channel: channel)

    let musicSubcriptionEventChannel = FlutterEventChannel(
      name: "plugins.misi.app/music_kit/music_subscription", binaryMessenger: registrar.messenger())
    ...
    let musicPlayerStateEventChannel = FlutterEventChannel(
      name: "plugins.misi.app/music_kit/player_state", binaryMessenger: registrar.messenger())
    musicPlayerStateEventChannel.setStreamHandler(
      MusicPlayerStreamHandler(musicPlayer: instance.musicPlayer))

    let musicPlayerQueueEventChannel = FlutterEventChannel(
      name: "plugins.misi.app/music_kit/player_queue", binaryMessenger: registrar.messenger())
    musicPlayerQueueEventChannel.setStreamHandler(
      MusicPlayerQueueStreamHandler(musicPlayer: instance.musicPlayer))
  }
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case .setQueueWithItems:
      ...
      setQueue(
        itemType: arguments["type"] as! String,
        itemObjects: arguments["items"] as! [ResourceObject],
